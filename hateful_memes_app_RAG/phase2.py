...
def run_phase2(phase1_excel: str, phase1_sheet: str,
    ...
    for i, row in enumerate(rows):
        ...
        if use_rag and rag_retriever is not None:
            examples = rag_retriever.get_examples(
                f"{description} {meme_text}", k=4)
            if examples:
                parts = ["\n\nHere are similar labeled examples for calibration:"]
                for i, ex in enumerate(examples, 1):
                    lbl = int(ex.get("label", 0))
                    lbl_str = "1 (hateful)" if lbl == 1 else "0 (not hateful)"
                    parts.append(
                        f"\nExample {i}:\n"
                        f"Image description: {ex.get('description', '')}\n"
                        f"Text on image: {ex.get('text', '')}\n"
                        f"Label: {lbl_str}")
                context_block = "\n".join(parts)
